...
class SupplierDetailModal extends React.Component {
  ...
  async refreshData() {
    ...
    if (!!id) {
      try {
        const data = await fetch(() => suppliers.get(id));
        this.setState({ data });
      } catch(err) {
        message.error(`Something went wrong while fetching details for supplier ${id}`);
      }
    }
  }
  ...
  async delete() {
    ...
    if (!!id) {
      try {
        await fetch(() => suppliers.remove(id));

        onCancel();
        refreshTableData();
        message.success(`Successfully deleted supplier ${id}`);
      } catch(err) {
        message.error(`Something went wrong while deleting supplier ${id}`);
      }
    }
  }
